function [Coefmatrix,A1] = RBFassemblemat(type,class,dt,Nx,A,sigma1,sigma2,r1,r2,lam1,lam2,...
            ...
            switch class
                case 0  % Merton
                    for i = 1:Nx+1
                        ...
                        B1(i,i) = (r1+lam1)*abs(x(i)-x(i))-(r1-0.5*sigma1^2-lam1*kappaM)*(0)...
                            -lam1*quadgk(fii,-zmax,zmax);
                        Integral = quadgk(fii,-zmax,zmax);
                        B2(i,i) = (r2+lam2)*abs(x(i)-x(i))-(r2-0.5*sigma2^2-lam2*kappaM)*(0)...
                    ...
                    for i = 2:Nx+1
                        for j = 1:i-1
                            ...
                            B1(i,j) = (r1+lam1)*abs(x(i)-x(j))-(r1-0.5*sigma1^2-lam1*kappaM)*(1)...
                                -lam1*quadgk(fij,-zmax,zmax);
                            B2(i,j) = (r2+lam2)*abs(x(i)-x(j))-(r2-0.5*sigma2^2-lam2*kappaM)*(1)...
                ...
                case 1  % Kou
                    for i = 1:Nx+1
                        ...
                        B1(i,i) = (r1+lam1)*abs(x(i)-x(i))-(r1-0.5*sigma1^2-lam1*kappaK)*(0)...
                            -lam1*quadgk(fii,-zmin,zmax);
                        B2(i,i) = (r2+lam2)*abs(x(i)-x(i))-(r2-0.5*sigma2^2-lam2*kappaK)*(0)...
                            -lam2*quadgk(fii,-zmin,zmax);
                    end
                    for i = 2:Nx+1
                        for j = 1:i-1
                            fij = @(z) abs(z+x(i)-x(j)).*(q*a2*exp(a2*z).*(z<0)+p*a1*exp(-a1*z).*(z>=0));
                            B1(i,j) = (r1+lam1)*abs(x(i)-x(j))-(r1-0.5*sigma1^2-lam1*kappaK)*(1)...
                                -lam1*quadgk(fij,-zmin,zmax);
                            B2(i,j) = (r2+lam2)*abs(x(i)-x(j))-(r2-0.5*sigma2^2-lam2*kappaK)*(1)...
            ...
            switch class
                case 0  % Merton
                    for i = 1:Nx+1
                        ...
                        zmax = sqrt(-2*(delt)^2*log(epsilon*delt*(sqrt(2*pi))/2))+mu;  % 定义数值积分的上界
                        B1(i,i) = (r1+lam1)*abs(x(i)-x(i)).^3-(r1-0.5*sigma1^2-lam1*kappaM)*0-0.5*sigma1^2*(6*0)...
                            -lam1*quadgk(fii,-zmax,zmax);
                        B2(i,i) = (r2+lam2)*abs(x(i)-x(i)).^3-(r2-0.5*sigma2^2-lam2*kappaM)*0-0.5*sigma2^2*(6*0)...
                            -lam2*quadgk(fii,-zmax,zmax);
                    end
                    for i = 2:Nx+1
                        for j = 1:i-1
                            fij = @(z) 1/(sqrt(2*pi)*delt)*abs(z+x(i)-x(j)).^3.*exp(-(z-mu).^2/2/(delt^2));
                            B1(i,j) = (r1+lam1)*abs(x(i)-x(j)).^3-(r1-0.5*sigma1^2-lam1*kappaM)*3*(x(i)-x(j)).^2 ...
                            -0.5*sigma1^2*(6*abs(x(i)-x(j)))-lam1*quadgk(fij,-zmax,zmax);
                            B2(i,j) = (r2+lam2)*abs(x(i)-x(j)).^3-(r2-0.5*sigma2^2-lam2*kappaM)*3*(x(i)-x(j)).^2 ...
                    ...
                    for i = 1:Nx
                        for j = i+1:Nx+1
                            ...
                            B1(i,j) = (r1+lam1)*abs(x(i)-x(j)).^3-(r1-0.5*sigma1^2-lam1*kappaM)*(-1)*3*(x(i)-x(j)).^2 ...
                                -0.5*sigma1^2*(6*abs(x(i)-x(j)))-lam1*quadgk(fji,-zmax,zmax);
                            B2(i,j) = (r2+lam2)*abs(x(i)-x(j)).^3-(r2-0.5*sigma2^2-lam2*kappaM)*(-1)*3*(x(i)-x(j)).^2 ...
                ...
                case 1  % Kou
                    for i = 1:Nx+1
                        ...
                        zmax = log(epsilon/p)/(1-a1);  % 定义数值积分的上界
                        zmin = -log(epsilon/q)/(1-a2);  % 定义数值积分的下界
                        B1(i,i) = (r1+lam1)*abs(x(i)-x(i)).^3-(r1-0.5*sigma1^2-lam1*kappaK)*0-0.5*sigma1^2*(6*0)...
                            -lam1*quadgk(fii,-zmin,zmax);
                        B2(i,i) = (r2+lam2)*abs(x(i)-x(i)).^3-(r2-0.5*sigma2^2-lam2*kappaK)*0-0.5*sigma2^2*(6*0)...
                            -lam2*quadgk(fii,-zmin,zmax);
                    end
                    for i = 2:Nx+1
                        for j = 1:i-1
                            fij = @(z) abs(z+x(i)-x(j)).^3.*(q*a2*exp(a2*z).*(z<0)+p*a1*exp(-a1*z).*(z>=0));
                            B1(i,j) = (r1+lam1)*abs(x(i)-x(j)).^3-(r1-0.5*sigma1^2-lam1*kappaK)*3*(x(i)-x(j)).^2 ...
                            -0.5*sigma1^2*(6*abs(x(i)-x(j)))-lam1*quadgk(fij,-zmax,zmax);
                            B2(i,j) = (r2+lam2)*abs(x(i)-x(j)).^3-(r2-0.5*sigma2^2-lam2*kappaK)*3*(x(i)-x(j)).^2 ...
                            -0.5*sigma2^2*(6*abs(x(i)-x(j)))-lam2*quadgk(fij,-zmax,zmax);
                        end
                    end
                    for i = 1:Nx
                        for j = i+1:Nx+1
                            ...
                            B1(i,j) = (r1+lam1)*abs(x(i)-x(j)).^3-(r1-0.5*sigma1^2-lam1*kappaK)*(-1)*3*(x(i)-x(j)).^2 ...
                            -0.5*sigma1^2*(6*abs(x(i)-x(j)))-lam1*quadgk(fji,-zmax,zmax);
                            B2(i,j) = (r2+lam2)*abs(x(i)-x(j)).^3-(r2-0.5*sigma2^2-lam2*kappaK)*(-1)*3*(x(i)-x(j)).^2 ...
                            -0.5*sigma2^2*(6*abs(x(i)-x(j)))-lam2*quadgk(fji,-zmax,zmax);
                        end
                    end
            end
