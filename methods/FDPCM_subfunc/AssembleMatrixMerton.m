function [B,F,Phi_star] = AssembleMatrixMerton(n,dt,dx,Nx,sigma1,sigma2,nM1,nM2,mM1,mM2,A,lam1,lam2,L,K,...
    ...
    % Assemble matrix B and vector F (at time layer n)
   
    % beta is the ratio of time-space partitioning
    beta = dt/(dx^2);

    %%%%%%%%%%%%%%%%% Assemble matrix B

    % Assemble matrices B1, B2
    B1 = zeros(Nx-1);
    B2 = zeros(Nx-1);
    for i = 2:Nx-2
    ...
    %%%%%%%%%%%%%% Assemble vector F, F=F0+(A-R)*Phi(n)+B*Phi_star
    %%%%% Assemble matrix A

    % Assemble matrices D1, D2
    a1 = ones(1,Nx-1);
    a2 = ones(1,Nx-1);
    for i = 1:Nx-1
    ...
    % Assemble vector F0, F0=FD-dt*lam*FI 
    % Assemble FD
    ...
    FI = dx/2*FI;
    % Assemble Fc (integral supplement termA(j,n))
    Fc = zeros(2*Nx-2,1);
    for j =1:Nx-1
