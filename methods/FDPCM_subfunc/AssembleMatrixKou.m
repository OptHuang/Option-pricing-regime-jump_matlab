function [B,F,Phi_star] = AssembleMatrixKou(n,dt,dx,Nx,sigma1,sigma2,nK1,nK2,mK1,mK2,A,lam1,lam2,L,K,...
    ...
    % Assemble matrix B and vector F for time layer n

    % beta is the ratio of time-space grid partition
    beta = dt/(dx^2);
    ...
    %%%%%%%%%%%%%%%% Form vector F, F=F0+(A-R)*Phi(n)+B*Phi_star
    %%%%% Form matrix A
    ...
    % Form vector F0, F0=FD-dt*lam*FI 
    % Form FD
